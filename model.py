...
class gcnnmodel(tf.Module):
    def __init__(self,inputFeatures_Num,outputSize):
        super(gcnnmodel, self).__init__()
        filters = [256,256]
        self.layer = []
        self.layer.append(GraphConvolution(shape=(inputFeatures_Num, filters[0]), dropout=0, name='layer1'))
        self.layer.append(GraphConvolution(shape=(filters[0], filters[1]), dropout=0, name='layer2'))
        self.layer.append(Dense(shape=(filters[1], outputSize), dropout=0, name='layer3'))
    @tf.function
